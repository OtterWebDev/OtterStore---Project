...
class MultiFormMixin(ContextMixin):
    ...
    def forms_valid(self, forms, form_name,):
        ...
        """ if group_name:
            form_valid_method = '%s_valid' % group_name
            form_names = group_name.split('_')
            if hasattr(self, form_valid_method):
                return getattr(self, form_valid_method)(forms['user'])
            else:      
                return HttpResponseRedirect(self.get_success_url(group_name)) """
...
class ProcessMultipleFormsView(ProcessFormView):
    ...
    def _process_grouped_forms(self, group_name,form_classes):
        ...
        if all([forms.get(form_name).is_valid() for form_name in form_names.keys()]):
